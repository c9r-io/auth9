...
Auth9 的 OIDC 身份引擎已内嵌于 auth9-core，无独立服务也无需切换开关，无额外配置项。早期版本的 `IDENTITY_BACKEND` 环境变量与 Keycloak 模式均已移除。
...
# [keycloak]  # 已移除：OIDC 引擎现在直接由 auth9-core 提供，不再支持 Keycloak 后端
...
services:
  auth9-core:
    environment:
      ...
      # OIDC 已内嵌于 auth9-core，无外部 IdP 配置项
```
