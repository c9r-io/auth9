...
| "No overlay/backdrop element found" | The overlay is **conditionally rendered** in React (`{isSidebarOpen && <div>}`), so it only exists in the DOM when the sidebar is open. Searching for overlay elements before opening the sidebar will find nothing. The overlay uses classes `bg-black/50 backdrop-blur-sm`, not "overlay" or "backdrop" as a class name. | First click the hamburger to open the sidebar, then query `document.querySelector('.fixed.inset-0.bg-black\\/50')` or check for any `fixed inset-0` element with `z-40`. The overlay is implemented at `dashboard.tsx:151-157`. |
