...
- 顶部 Hero 区域（标题、描述、CTA 按钮 — 含「阅读文档」链接到 `/docs`）
- 功能介绍卡片区域（Feature Cards）
- 顶部 Navbar（Logo、导航链接、控件区、登录按钮）
- 底部 Footer（Copyright + `/privacy`、`/terms` 链接）
...
| # | 场景 | 状态 | 测试日期 | 测试人员 | 备注 |
|---|------|------|----------|----------|------|
| 1 | 功能卡片入口可见性（初始状态） | ✅ | 2026-03-10 | opencode | |
| 2 | 功能卡片悬停效果（回归 Ticket #1）| ✅ | 2026-03-10 | opencode | **已知 Bug 回归项，必测** - 已修复，无消失问题 |
| 3 | Navbar CTA 按钮与控件可点击性 | ✅ | 2026-03-10 | opencode | |
| 4 | 入场动画不干扰内容可见性 | ✅ | 2026-03-10 | opencode | 动画结束后 opacity 保持 1 |
