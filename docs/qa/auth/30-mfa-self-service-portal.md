...
> **环境清洁度**: 测试初始状态（Email OTP 显示 "Not enabled"、TOTP 显示 "Not set up"）要求环境干净。如果之前的测试已启用过 MFA，状态可能残留。
>
> **必须在测试前运行**：`./scripts/reset-docker.sh`
>
> 此脚本会重置数据库和种子数据，确保测试用户的 MFA 状态为初始未启用状态。不运行此脚本时，Email OTP 状态可能显示 "Enabled"（环境脏数据），导致误报。

### 初始状态
- 用户已登录 Portal，进入 Dashboard
- 用户尚未启用 TOTP
- **环境已通过 `./scripts/reset-docker.sh` 重置为干净状态**
...
- 页面显示四个卡片区域：
  ...
  - **Email OTP**：状态标签显示「Not enabled」（灰色），可见「Enable Email OTP」按钮
