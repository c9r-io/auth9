...
**开关控制**（双重检查）：
- **系统级**：`BrandingConfig.email_otp_enabled` 控制全局是否启用，默认 `false`
- **用户级**：`users.email_otp_enabled` 控制用户个人是否启用，默认 `false`
- Email OTP 登录需**两者均为 true** 才能成功验证。发送验证码端点不检查用户级状态（防枚举）
- 用户级开关详见 [31-email-otp-user-toggle.md](./31-email-otp-user-toggle.md)
