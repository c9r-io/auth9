...
> **注意**: 登录表单的 email 和 password 输入框已设置 `required` HTML 属性。如果浏览器原生验证未触发，检查是否 JavaScript/React 的表单提交逻辑（如 `useFetcher` 或 `form.submit()`）绑定了按钮的 `onClick` 事件而绕过了 HTML 表单验证。React Router 的 `<Form>` 组件默认通过标准表单提交，会触发 `required` 验证；但如果前端代码使用了 `fetcher.submit()` 等编程式提交方式，则会跳过浏览器原生验证。
